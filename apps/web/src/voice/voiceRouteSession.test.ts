...
function createHarness(context = fakeContext()) {
...
describe("voice route session", () => {
  ...
  it("closes primed playback immediately when voice ends", async () => {
    const { context, session } = createHarness();
    session.retainPlayback();

    await session.ensurePlayback();
    session.closePlayback();

    expect(context.close).toHaveBeenCalledOnce();
    expect(session.getPlayback()).toBeNull();
  });

  it("keeps composer send state through StrictMode route handoff", () => {
    const { cancelled, scheduled, session } = createHarness();
